...
import { createProvenanceService } from '../provenance-service';
// Mock localStorage
...
describe('ProvenanceService', () => {
    ...
    describe('createProvenance', () => {
        ...
        it('should include TW Core profile', () => {
            ...
            expect(provenance.meta?.profile).toContain('https://twcore.mohw.gov.tw/ig/twcore/StructureDefinition/Provenance-twcore');
        });
        ...
        it('should include data source type', () => {
            ...
            const sourceEntity = provenance.entity?.find(e => e.what.identifier?.value === 'patient-upload');
    ...
    describe('Convenience Methods', () => {
        ...
        it('should record data creation provenance', async () => {
            const provenance = await provenanceService.recordDataCreation('Observation/obs-123', 'Blood Glucose Measurement', 'internal', 'Routine lab work');
            expect(provenance.activity?.coding[0].code).toBe('CREATE');
        ...
        it('should record data update provenance', async () => {
            const provenance = await provenanceService.recordDataUpdate('Observation/obs-123', 'Updated Blood Glucose', 'Observation/obs-122', 'Correction of measurement error');
        ...
        it('should record derivation provenance', async () => {
            const provenance = await provenanceService.recordDerivation('Observation/egfr-result', 'Calculated eGFR', [
        ...
        it('should record cross-hospital exchange provenance', async () => {
            const provenance = await provenanceService.recordCrossHospitalExchange('Observation/ext-123', 'External Lab Result', { id: 'org-external', name: 'External Hospital' }, 'Patient referral data');
            expect(provenance.activity?.coding[0].code).toBe('CREATE');
            // Check data source
            const sourceEntity = provenance.entity?.find(e => e.what.identifier?.value === 'cross-hospital');
            expect(sourceEntity).toBeDefined();
            // Check external organization agent
            const orgAgent = provenance.agent.find(a => a.who.reference === 'Organization/org-external');
            expect(orgAgent).toBeDefined();
            const records = provenanceService.getProvenanceRecords();
            expect(records.length).toBe(1);
        });
        it('should record patient upload provenance', async () => {
            const provenance = await provenanceService.recordPatientUpload('DocumentReference/doc-123', 'Patient Uploaded Document', 'patient-456', 'John Doe', 'Patient-provided medical history');
            expect(provenance.activity?.coding[0].code).toBe('CREATE');
            // Check patient as author
            const patientAgent = provenance.agent.find(a => a.who.reference === 'Patient/patient-456');
        ...
        it('should record provenance with signature', async () => {
            ...
            const provenance = await provenanceService.recordWithSignature('DiagnosticReport/report-123', 'Signed Diagnostic Report', 'VERIFY', signatureData, 'verification');
            expect(provenance.signature).toBeDefined();
            expect(provenance.signature?.[0].data).toBe(signatureData);
            const records = provenanceService.getProvenanceRecords();
            expect(records.length).toBe(1);
    ...
    describe('Lineage Report', () => {
        beforeEach(async () => {
            ...
            await provenanceService.recordDataCreation('Observation/target-1', 'Original Observation', 'internal');
            await provenanceService.recordDerivation('Observation/target-1', 'Derived Result', [{ reference: 'Observation/source-1', display: 'Source 1' }], 'Derivation');
        });
    ...
    describe('Data Source Types', () => {
        ...
        test.each(dataSources)('should handle $source data source', ({ source, expectedDisplay }) => {
            ...
            const sourceEntity = provenance.entity?.find(e => e.what.identifier?.value === source);
            expect(sourceEntity).toBeDefined();
    ...
    describe('Agent Roles', () => {
        ...
        test.each(roles)('should handle %s agent role', (role) => {
            ...
            const practitionerAgent = provenance.agent.find(a => a.who.reference === 'Practitioner/prac-123');
