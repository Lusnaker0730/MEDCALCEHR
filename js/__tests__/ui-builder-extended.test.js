...
describe('UI Builder Extended Tests', () => {
    ...
    describe('setRadioValue', () => {
        test('sets radio to checked and dispatches change event', () => {
            ...
            const radioB = document.getElementById('r-b');
    ...
    describe('initializeComponents - deeper tests', () => {
        describe('Copy Report button', () => {
            ...
            test('copy button handles clipboard failure gracefully', async () => {
                ...
                copyBtn.click();
                await new Promise(resolve => setTimeout(resolve, 50));
                expect(logger.error).toHaveBeenCalledWith('Failed to copy report', expect.objectContaining({ error: expect.any(String) }));
        ...
        describe('Range slider value display', () => {
            test('updates value display when slider input event fires', () => {
                ...
                uiBuilder.initializeComponents(container);
                const slider = container.querySelector('#test-slider');
                const valueDisplay = container.querySelector('#test-slider-value');
            ...
            test('preserves unit text when updating slider value', () => {
                ...
                uiBuilder.initializeComponents(container);
                const slider = container.querySelector('#pct-slider');
                const valueDisplay = container.querySelector('#pct-slider-value');
                slider.value = '85';
                slider.dispatchEvent(new Event('input'));
        ...
        describe('Checkbox visual feedback', () => {
            test('adds selected class when checkbox is checked', () => {
                ...
                uiBuilder.initializeComponents(container);
                const cbX = container.querySelector('input[value="x"]');
                // Check the checkbox
                cbX.checked = true;
            ...
            test('initializes selected class for pre-checked checkboxes', () => {
                ...
                uiBuilder.initializeComponents(container);
                const cb = container.querySelector('input[value="pre"]');
        ...
        describe('Unit toggle initialization', () => {
            test('handles invalid JSON in data-unit-toggle gracefully', () => {
                ...
                expect(logger.warn).toHaveBeenCalledWith('Failed to parse unit toggle config', expect.objectContaining({ error: expect.any(String) }));
            });
        });
    });
    // =========================================================================
