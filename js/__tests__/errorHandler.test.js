...
describe('Error Handler Module', () => {
    ...
    describe('displayError', () => {
        ...
        test('should render error message in container', () => {
            ...
            expect(container.querySelector('.error-content')?.textContent?.trim()).toBe('A friendly message');
        });
        ...
        test('should use getUserFriendlyMessage for FHIRDataError', () => {
            ...
            expect(content).toBe('Unable to retrieve patient data from EHR system. Please check connection or enter data manually.');
        });
        ...
        test('should use generic message for unknown Error type', () => {
            ...
            displayError(container, error);
            const content = container.querySelector('.error-content')?.textContent?.trim();
            expect(content).toBe('Calculator encountered an error. Please refresh the page and try again or contact support.');
        });
    ...
    describe('setupGlobalErrorHandler', () => {
        ...
        test('should register unhandledrejection event listener (line 214)', () => {
            const addEventSpy = jest.spyOn(window, 'addEventListener');
            setupGlobalErrorHandler();
            const rejectionCalls = addEventSpy.mock.calls.filter(call => call[0] === 'unhandledrejection');
        ...
        test('error event handler should call logError with context', () => {
            ...
            const addEventSpy = jest.spyOn(window, 'addEventListener').mockImplementation(((event, handler) => {
                if (event === 'error') {
