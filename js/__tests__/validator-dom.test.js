...
import { setupLiveValidation, setupFormValidation, ValidationRules } from '../validator.js';
describe('setupLiveValidation', () => {
    ...
    test('returns early without error when inputElement is null', () => {
        expect(() => {
            setupLiveValidation(null, ValidationRules.age);
        }).not.toThrow();
    });
    // -----------------------------------------------------------------------
    ...
    test('blur with invalid value adds "invalid" class, aria-invalid, and error span', () => {
        ...
        const rule = { required: true, min: 0, max: 150, message: 'Age must be between 0-150 years' };
    ...
    test('blur with valid value removes "invalid" class, aria-invalid, and error span', () => {
        ...
        input.type = 'number';
        container.appendChild(input);
        const rule = { required: true, min: 0, max: 150, message: 'Age out of range' };
        setupLiveValidation(input, rule);
    ...
    test('input event removes "invalid" class and error span without re-validating', () => {
        ...
        input.type = 'number';
        container.appendChild(input);
        const rule = { required: true, min: 0, max: 150 };
        setupLiveValidation(input, rule);
    ...
    test('reuses existing error-text span instead of creating a duplicate', () => {
        ...
        input.type = 'number';
        container.appendChild(input);
        const rule = { required: true, min: 0, max: 150, message: 'Out of range' };
        setupLiveValidation(input, rule);
        // First blur with invalid value
        input.value = '-5';
        input.dispatchEvent(new Event('blur'));
        const firstSpan = input.nextElementSibling;
        ...
        const secondSpan = input.nextElementSibling;
    ...
    test('error span is inserted immediately after the input element', () => {
        ...
        const rule = { required: true, min: 0, max: 100, message: 'Invalid' };
        setupLiveValidation(input, rule);
        input.value = '-1';
        input.dispatchEvent(new Event('blur'));
...
describe('setupFormValidation', () => {
    ...
    test('returns early without error when formElement is null', () => {
        expect(() => {
            setupFormValidation(null, { age: ValidationRules.age });
        }).not.toThrow();
    });
    // -----------------------------------------------------------------------
    ...
    test('sets up live validation for each input found by schema key as ID', () => {
        ...
        expect(ageInput.nextElementSibling.textContent).toBe('Age out of range');
        weightInput.value = '0';
        weightInput.dispatchEvent(new Event('blur'));
        expect(weightInput.classList.contains('invalid')).toBe(true);
        expect(weightInput.nextElementSibling.textContent).toBe('Weight out of range');
    });
    // -----------------------------------------------------------------------
