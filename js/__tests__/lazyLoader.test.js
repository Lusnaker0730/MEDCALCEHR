...
describe('Lazy Loader Module', () => {
    ...
    describe('loadCSS', () => {
        ...
        test('should set the id on the link element when provided', async () => {
            ...
            const link = document.head.querySelector('#theme-css');
            expect(link).not.toBeNull();
    ...
    describe('loadCalculator', () => {
        test('should dynamically import the calculator module', async () => {
            ...
            await expect(loadCalculator('nonexistent-calc')).rejects.toThrow('Calculator "nonexistent-calc" not found');
            expect(console.error).toHaveBeenCalledWith('Failed to load calculator: nonexistent-calc', expect.anything());
        });
    ...
    describe('ImageLazyLoader', () => {
        describe('constructor with IntersectionObserver available', () => {
            test('should create an IntersectionObserver with default options', async () => {
                ...
                expect(mockObserverInstance.options).toEqual(expect.objectContaining({
                    root: null,
                    rootMargin: '50px',
                    threshold: 0.01
                }));
        ...
        describe('loadImage — onload and onerror paths', () => {
            ...
            test('should add error class and log error when image fails to load', async () => {
                ...
                expect(console.error).toHaveBeenCalledWith('Failed to load image: /images/broken.jpg');
            });
        });
    ...
    describe('loadOnIdle', () => {
        test('should resolve with the imported module via idle callback', async () => {
            ...
            const importFn = jest.fn().mockResolvedValue(fakeModule);
        ...
        test('should reject when the import function fails', async () => {
            ...
            window.requestIdleCallback = mockRIC;
            const { loadOnIdle } = await importLazyLoader();
            const importFn = jest.fn().mockRejectedValue(new Error('Network failed'));
