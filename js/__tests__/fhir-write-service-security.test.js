...
jest.mock('../provenance-service', () => ({
    provenanceService: {
        recordCalculation: jest.fn().mockResolvedValue({ id: 'prov-1' }),
    },
}));
...
describe('FHIRWriteService — Security Labels Integration', () => {
    ...
    beforeEach(() => {
        ...
        window.MEDCALC_CONFIG = { enableWriteBack: true };
    ...
    test('buildObservation produces a resource with meta.security containing Confidentiality label', () => {
        ...
        const service = fhirWriteService;
        ...
        const confLabel = observation.meta.security.find((s) => s.system === 'http://terminology.hl7.org/CodeSystem/v3-Confidentiality');
