...
const mockCheckStaleness = jest.fn().mockReturnValue(null);
const mockTrackObservation = jest.fn();
const mockDataStalenessTracker = jest.fn();
const mockLogResourceRead = jest.fn().mockResolvedValue(undefined);
const mockRecordCalculation = jest.fn().mockResolvedValue({});
const mockRecordDerivation = jest.fn().mockResolvedValue({});
const mockGetProvenanceForTarget = jest.fn().mockReturnValue([]);
const mockGenerateLineageReport = jest.fn().mockReturnValue({});
...
const mockGetObservationValue = jest.fn().mockReturnValue(null);
const mockGetPatientConditions = jest.fn().mockResolvedValue([]);
const mockGetMedicationRequests = jest.fn().mockResolvedValue([]);
const mockCalculateAge = jest.fn().mockReturnValue(30);
const mockIsRestrictedResource = jest.fn().mockReturnValue(false);
const mockGetLoincName = jest.fn();
const mockGetMeasurementType = jest.fn().mockReturnValue('concentration');
const mockIsValidLoincCode = jest.fn().mockReturnValue(true);
const mockGetTextNameByLoinc = jest.fn().mockReturnValue(null);
const mockUnitConvert = jest.fn().mockReturnValue(null);
const mockSetInputValue = jest.fn();
const mockGetActiveAdapter = jest.fn().mockReturnValue(null);
// Mock dependencies
jest.mock('../logger.js', () => ({
...
describe('TW Core Observation', () => {
    ...
    describe('processObservation TW Core profile detection', () => {
        ...
        function callProcessObservation(obs, code) {
            return service.processObservation(obs, code, {});
        }
