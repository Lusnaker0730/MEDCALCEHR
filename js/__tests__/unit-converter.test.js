...
describe('UnitConverter', () => {
    ...
    describe('Edge Cases (extended)', () => {
        ...
        test('should return null for null input', () => {
            expect(UnitConverter.convert(null, 'kg', 'lbs', 'weight')).toBeNull();
        });
        test('should return null for undefined input', () => {
            expect(UnitConverter.convert(undefined, 'kg', 'lbs', 'weight')).toBeNull();
        });
    ...
    describe('setInputValue', () => {
        ...
        test('should do nothing for null value', () => {
            input.value = '50';
            UnitConverter.setInputValue(input, null, 'kg');
            expect(input.value).toBe('50');
        });
        test('should do nothing for undefined value', () => {
            input.value = '50';
            UnitConverter.setInputValue(input, undefined, 'kg');
            expect(input.value).toBe('50');
        });
        test('should do nothing for null input element', () => {
            // Should not throw
            expect(() => UnitConverter.setInputValue(null, 100, 'kg')).not.toThrow();
        });
    });
    // ==========================================
    // DOM: autoEnhance
    ...
    describe('autoEnhance', () => {
        ...
        test('should auto-enhance input with id=temperature', () => {
            ...
            container.appendChild(input);
            UnitConverter.autoEnhance(container);
            const btn = container.querySelector('.unit-toggle-btn');
            expect(btn).not.toBeNull();
            expect(btn.dataset.currentUnit).toBe('C');
        });
    ...
    describe('CSS stylesheet injection', () => {
        test('should add unit-converter-styles link element to head', () => {
            ...
            if (link) {
                ...
                expect(link.href).toContain('unit-converter.css');
            }
        });
    });
});
