...
// Mock helpers
const createMockGetters = (inputs) => {
    return {
        getValue: (id) => inputs[id] ?? null,
        getStdValue: (id, _unit) => inputs[id] ?? null, // Assume inputs are already in std units
        getRadioValue: (_id) => '', // Not used in current logic
        getCheckboxValue: (_id) => false // Added mock
    };
};
