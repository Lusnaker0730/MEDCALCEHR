...
describe('SIRS Criteria Calculator (SaMD Protocol Verification)', () => {
    ...
    test('TC-002: Boundary Analysis - Temperature', () => {
        ...
        const resHigh = sirsCalculation({
            'temp': 38.1, 'hr': 80, 'rr': 16, 'wbc': 7.5, 'infection': 'no', 'hypotension': 'no'
        });
        expect(resHigh[0].alertPayload.criteriaCount).toBe(1);
        // Low Temp Boundary < 36
        const resLow = sirsCalculation({
            'temp': 35.9, 'hr': 80, 'rr': 16, 'wbc': 7.5, 'infection': 'no', 'hypotension': 'no'
        });
        expect(resLow[0].alertPayload.criteriaCount).toBe(1);
        // Normal Temp Boundary 36 & 38
        const resNorm1 = sirsCalculation({ 'temp': 36.0, 'hr': 80, 'rr': 16, 'wbc': 7.5 });
        expect(resNorm1[0].alertPayload.criteriaCount).toBe(0);
        const resNorm2 = sirsCalculation({ 'temp': 38.0, 'hr': 80, 'rr': 16, 'wbc': 7.5 });
        expect(resNorm2[0].alertPayload.criteriaCount).toBe(0);
    });
    test('TC-003: Boundary Analysis - Heart Rate', () => {
        ...
        expect(resNorm[0].alertPayload.criteriaCount).toBe(0);
    });
    test('TC-004: Boundary Analysis - WBC / Bands', () => {
        ...
        expect(resBands[0].alertPayload.criteriaCount).toBe(1);
    });
    test('TC-005: Sepsis Diagnosis (2 Criteria + Infection)', () => {
        ...
        expect(result[0].alertPayload.criteriaCount).toBe(2);
