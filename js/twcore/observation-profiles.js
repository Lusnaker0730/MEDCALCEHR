 ...
 * 4. null (unknown)
 */
export function getTWCoreObservationProfile(loincCode) {
 ...
 */
export function isVitalSignCode(loincCode) {
    const primaryCode = loincCode.split(',')[0].trim();
 ...
 */
export function getTWCoreObservationCategory(loincCode) {
    const primaryCode = loincCode.split(',')[0].trim();
    if (VITAL_SIGN_LOINC_CODES.has(primaryCode)) {
        return 'vital-signs';
    }
