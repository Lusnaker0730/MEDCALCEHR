...
export const cpis: Calculator = {
    ...
    initialize: (client: FHIRClient, patient: Patient, container: HTMLElement): void => {
        ...
        const calculate = () => {
            ...
            const resultBox = container.querySelector('#cpis-result-box') as HTMLElement;
        ...
        if (client) {
            ...
            getMostRecentObservation(client, LOINC_CODES.TEMPERATURE).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity) {
                    ...
                    if (unit && (unit.includes('F') || unit.includes('degF'))) {
                        tempC = ((tempC - 32) * 5) / 9;
                    }
            ...
            // WBC
            getMostRecentObservation(client, LOINC_CODES.WBC).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity) {
                    const wbc = obs.valueQuantity.value; // assuming 10^3/uL or K/uL which are standard
            ...
            Promise.all([
                ...
                getMostRecentObservation(client, LOINC_CODES.CULTURE) // This was culture in original, likely irrelevant here but kept structure
            ]).then(([pao2Obs]) => {
                // Logic for PaO2/FiO2 if available directly
                // Or fetch PaO2 and FiO2 separately
            });
        }
