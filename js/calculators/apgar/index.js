...
export const apgarScore = createRadioScoreCalculator({
    ...
    customResultRenderer: (score, sectionScores) => {
        const riskLevels = [
            { minScore: 7, maxScore: 10, label: 'Reassuring (Normal)', severity: 'success' },
            { minScore: 4, maxScore: 6, label: 'Moderately Abnormal', severity: 'warning' },
        ...
        const breakdownHTML = components.map(c => `<span style="margin-right: 10px;">${c.name}: ${sectionScores[c.key] ?? '-'}</span>`).join('');
        let interpretation = '';
        if (score >= 7) {
