    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            groups.forEach(group => {
                const selected = container.querySelector(`input[name="${group}"]:checked`) as HTMLInputElement;
                if (selected) {
        ...
        const setRadioFromValue = (groupName: string, value: number | null, ranges: { condition: (v: number) => boolean, value: string }[], displayValue: string, unit: string) => {
