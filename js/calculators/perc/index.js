    ...
    initialize: function (client, patient) {
        ...
        document.getElementById('calculate-perc').addEventListener('click', () => {
            ...
            if (criteriaMet.length === 0) {
                resultEl.innerHTML =
                    '<p class="result-positive"><strong>PERC Negative:</strong> PE may be ruled out. No further testing is indicated if pre-test probability is low (≤15%).</p>';
            } else {
                resultEl.innerHTML =
                    '<p class="result-negative"><strong>PERC Positive:</strong> The rule is positive. PE is not ruled out. Further testing (e.g., D-dimer, imaging) should be considered.</p>';
            }
