...
export const mewsScore = {
    ...
    initialize(client, patient, container) {
        ...
        const calculate = () => {
            ...
            // 找到對應的風險等級
            const riskLevel = config.riskLevels.find(r => score >= r.minScore && score <= r.maxScore) || config.riskLevels[config.riskLevels.length - 1];
            // 特殊警告
            let criticalWarning = '';
            if (hasCriticalParam) {
                criticalWarning = `
        ...
        if (client) {
            ...
            getMostRecentObservation(client, LOINC_CODES.SYSTOLIC_BP).then(obs => {
                if (obs?.valueQuantity) {
                    ...
                    if (sbp <= 70)
                        setRadioValue('mews-sbp', '3', obs, LOINC_CODES.SYSTOLIC_BP, 'Systolic BP');
                    else if (sbp <= 80)
                        setRadioValue('mews-sbp', '2', obs, LOINC_CODES.SYSTOLIC_BP, 'Systolic BP');
                    else if (sbp <= 100)
                        setRadioValue('mews-sbp', '1', obs, LOINC_CODES.SYSTOLIC_BP, 'Systolic BP');
                    else if (sbp <= 199)
                        setRadioValue('mews-sbp', '0', obs, LOINC_CODES.SYSTOLIC_BP, 'Systolic BP');
                    else
                        setRadioValue('mews-sbp', '2', obs, LOINC_CODES.SYSTOLIC_BP, 'Systolic BP');
                }
            ...
            getMostRecentObservation(client, LOINC_CODES.HEART_RATE).then(obs => {
                if (obs?.valueQuantity) {
                    ...
                    if (hr < 40)
                        setRadioValue('mews-hr', '2', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                    else if (hr <= 50)
                        setRadioValue('mews-hr', '1', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                    else if (hr <= 100)
                        setRadioValue('mews-hr', '0', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                    else if (hr <= 110)
                        setRadioValue('mews-hr', '1', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                    else if (hr <= 129)
                        setRadioValue('mews-hr', '2', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                    else
                        setRadioValue('mews-hr', '3', obs, LOINC_CODES.HEART_RATE, 'Heart Rate');
                }
            }).catch(e => console.warn(e));
            // 呼吸率
            getMostRecentObservation(client, LOINC_CODES.RESPIRATORY_RATE).then(obs => {
                if (obs?.valueQuantity) {
                    ...
                    if (rr < 9)
                        setRadioValue('mews-rr', '2', obs, LOINC_CODES.RESPIRATORY_RATE, 'Resp Rate');
                    else if (rr <= 14)
                        setRadioValue('mews-rr', '0', obs, LOINC_CODES.RESPIRATORY_RATE, 'Resp Rate');
                    else if (rr <= 20)
                        setRadioValue('mews-rr', '1', obs, LOINC_CODES.RESPIRATORY_RATE, 'Resp Rate');
                    else if (rr <= 29)
                        setRadioValue('mews-rr', '2', obs, LOINC_CODES.RESPIRATORY_RATE, 'Resp Rate');
                    else
                        setRadioValue('mews-rr', '3', obs, LOINC_CODES.RESPIRATORY_RATE, 'Resp Rate');
                }
            }).catch(e => console.warn(e));
            // 體溫
