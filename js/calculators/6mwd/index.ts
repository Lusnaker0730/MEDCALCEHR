...
export const sixMwd: Calculator = {
    id: '6mwd',
    ...
    initialize: function (client: FHIRClient | null, patient: Patient | null, container: HTMLElement): void {
        ...
        const resultBox = container.querySelector('#mwd6-result') as HTMLElement;

        const calculate = () => {
            const age = parseInt(ageEl.value);
            const genderRadio = container.querySelector('input[name="mwd6-gender"]:checked') as HTMLInputElement;
        ...
        if (patient && patient.birthDate) {
            ageEl.value = calculateAge(patient.birthDate).toString();
        }

        if (patient && patient.gender) {
            const genderRadio = container.querySelector(`input[name="mwd6-gender"][value="${patient.gender}"]`) as HTMLInputElement;
            if (genderRadio) {
        ...
        if (client) {
            getMostRecentObservation(client, LOINC_CODES.HEIGHT).then((obs: Observation | null) => {
