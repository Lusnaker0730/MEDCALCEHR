...
export const growthChart = {
    ...
    initialize: function (client, patient, container) {
        ...
        function calculateZScore(ageMonths, value, cdcDataArray) {
        ...
        async function getGrowthData(client, patient) {
            ...
            const requests = Object.entries(loincCodes).map(([key, code]) => client.patient
                ...
                results.forEach(({ key, response }) => {
                    if (response.entry) {
                        data[key] = response.entry
                            .map((item) => ({
                            ageMonths: (new Date(item.resource.effectiveDateTime).getTime() - new Date(patient.birthDate).getTime()) /
