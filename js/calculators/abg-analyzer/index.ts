...
export const abgAnalyzer: Calculator = {
    id: 'abg-analyzer',
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const resultBox = container.querySelector('#abg-result') as HTMLElement;
        ...
        const interpret = () => {
            ...
            ['ph', 'pco2', 'hco3'].forEach(key => {
                const val = parseFloat(fields[key as keyof typeof fields].value);
                if (isNaN(val)) missingRequired = true;
                vals[key] = val;
            });

            ['sodium', 'chloride', 'albumin'].forEach(key => {
                vals[key] = parseFloat(fields[key as keyof typeof fields].value);
            });
        ...
        // FHIR Auto-populate
        if (client) {
            const mapping: Record<string, HTMLInputElement> = {
                '11558-4': fields.ph,
                '11557-6': fields.pco2,
                '14627-4': fields.hco3,
            ...
            Object.entries(mapping).forEach(([code, field]) => {
                getMostRecentObservation(client, code).then((obs: Observation | null) => {
