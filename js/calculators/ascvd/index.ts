...
export const ascvd: Calculator = {
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        if (patient && patient.gender) {
            ...
            const genderRadio = container.querySelector(`input[name="ascvd-gender"][value="${genderValue}"]`) as HTMLInputElement;
            if (genderRadio) {
        ...
        if (client) {
            ...
            getMostRecentObservation(client, LOINC_CODES.CHOLESTEROL_TOTAL).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity) {
                    tcInput.value = obs.valueQuantity.value.toFixed(0);
                }
            }).catch(console.log);

            getMostRecentObservation(client, LOINC_CODES.HDL).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity) {
        ...
        const calculate = () => {
            ...
            if (knownAscvdCheckbox.checked) {
                ...
                const genderRadio = container.querySelector('input[name="ascvd-gender"]:checked') as HTMLInputElement;
                const raceRadio = container.querySelector('input[name="ascvd-race"]:checked') as HTMLInputElement;
                const htnRadio = container.querySelector('input[name="ascvd-htn"]:checked') as HTMLInputElement;
                const dmRadio = container.querySelector('input[name="ascvd-dm"]:checked') as HTMLInputElement;
                const smokerRadio = container.querySelector('input[name="ascvd-smoker"]:checked') as HTMLInputElement;
        ...
        (container.querySelector('#calculate-therapy-impact') as HTMLElement).addEventListener('click', () => {
            ...
            // Statin
            if ((container.querySelector('#statin-therapy') as HTMLInputElement).checked) {
