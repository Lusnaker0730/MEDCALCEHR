...
export const graceAcs: Calculator = {
    id: 'grace-acs',
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            const sbp = parseInt(sbpInput.value);
            const creatinine = parseFloat(creatinineInput.value);

            const killipRadio = container.querySelector('input[name="grace-killip"]:checked') as HTMLInputElement;
            const arrestRadio = container.querySelector('input[name="grace-cardiac-arrest"]:checked') as HTMLInputElement;
            const stRadio = container.querySelector('input[name="grace-st-deviation"]:checked') as HTMLInputElement;
            const enzymesRadio = container.querySelector('input[name="grace-cardiac-enzymes"]:checked') as HTMLInputElement;
            ...
            const resultBox = container.querySelector('#grace-result') as HTMLElement;
        ...
        if (client) {
            ...
            getMostRecentObservation(client, LOINC_CODES.SYSTOLIC_BP).then((obs: Observation | null) => {
                if (obs?.valueQuantity) {
                    const sbpInput = container.querySelector('#grace-sbp') as HTMLInputElement;
                    sbpInput.value = Math.round(obs.valueQuantity.value).toString();
                }
            });
