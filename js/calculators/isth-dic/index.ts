...
export const isthDic: Calculator = {
    ...
    initialize: function (client: FHIRClient | null, patient: Patient | null, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            const resultBox = container.querySelector('#isth-dic-result') as HTMLElement;
        ...
        const setRadioFromValue = (groupName: string, value: number | null, ranges: { condition: (v: number) => boolean, value: string }[]) => {
        ...
        // FHIR Integration
        if (client) {
            getMostRecentObservation(client, '26515-7').then((obs: Observation | null) => { // Platelets
                if (obs && obs.valueQuantity) {
                    plateletInput.value = obs.valueQuantity.value.toFixed(0);
                    plateletInput.dispatchEvent(new Event('input'));
                }
            });
            getMostRecentObservation(client, '48065-7').then((obs: Observation | null) => { // D-dimer
                if (obs && obs.valueQuantity) {
                    ddimerInput.value = obs.valueQuantity.value.toFixed(2);
                    ddimerInput.dispatchEvent(new Event('input'));
                }
            });
            getMostRecentObservation(client, LOINC_CODES.PT).then((obs: Observation | null) => { // PT
                if (obs && obs.valueQuantity) {
