...
export const kawasaki: Calculator = {
    id: 'kawasaki',
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            const fever = (container.querySelector('input[name="kawasaki-fever"]:checked') as HTMLInputElement)?.value === '1';
            ...
            features.forEach(feature => {
                if ((container.querySelector(`input[name="${feature}"]:checked`) as HTMLInputElement)?.value === '1') {
                    featureCount++;
                }
            });

            const resultBox = container.querySelector('#kawasaki-result') as HTMLElement;
