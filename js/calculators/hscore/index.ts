...
export const hscore: Calculator = {
    id: 'hscore',
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            const resultBox = container.querySelector('#hscore-result') as HTMLElement;
        ...
        const setRadioFromValue = (groupName: string, value: number | null | undefined, ranges: { condition: (v: number) => boolean, value: string }[]) => {
        ...
        if (client) {
            Promise.all([
                ...
                if (hgb && hgb.valueQuantity && hgb.valueQuantity.value <= 9.2) cytopeniaCount++;
                if (wbc && wbc.valueQuantity && wbc.valueQuantity.value <= 5) cytopeniaCount++; // Assuming K/uL
                if (platelets && platelets.valueQuantity && platelets.valueQuantity.value <= 110) cytopeniaCount++; // Assuming K/uL
            ...
            getMostRecentObservation(client, '2276-4').then((obs: Observation | null) => { // Ferritin
                if (obs && obs.valueQuantity) {
            ...
            getMostRecentObservation(client, LOINC_CODES.TRIGLYCERIDES).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity) {
            ...
            getMostRecentObservation(client, '3255-7').then((obs: Observation | null) => { // Fibrinogen
                if (obs && obs.valueQuantity) {
