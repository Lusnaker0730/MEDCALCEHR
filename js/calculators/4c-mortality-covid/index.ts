...
export const fourCMortalityCovid: Calculator = {
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            const resultBox = container.querySelector('#four-c-result') as HTMLElement;
        ...
        if (client) {
            ...
            getMostRecentObservation(client, LOINC_CODES.OXYGEN_SATURATION).then((obs: Observation | null) => {
                if (obs?.valueQuantity) {
                    const val = obs.valueQuantity.value;
            ...
            getMostRecentObservation(client, LOINC_CODES.GCS).then((obs: Observation | null) => {
                if (obs?.valueQuantity) {
                    const val = obs.valueQuantity.value;
            ...
            getMostRecentObservation(client, LOINC_CODES.CRP).then((obs: Observation | null) => {
                if (obs?.valueQuantity) {
                    const val = obs.valueQuantity.value;
