...
export const fourCMortalityCovid = {
    ...
    initialize: async function(client) {
        ...
        const selectButtonByValue = (groupName, value) => {
            ...
            if (groupName === 'age') {
                if (value < 50) selectedButton = buttons[0];
                else if (value <= 59) selectedButton = buttons[1];
                else if (value <= 69) selectedButton = buttons[2];
                else if (value <= 79) selectedButton = buttons[3];
                else selectedButton = buttons[4];
            } else if (groupName === 'sex') {
                if (value === 'female') selectedButton = buttons[0];
                else if (value === 'male') selectedButton = buttons[1];
            } else if (groupName === 'comorbidities') {
                if (value === 0) selectedButton = buttons[0];
                else if (value === 1) selectedButton = buttons[1];
                else selectedButton = buttons[2];
            } else if (groupName === 'resp_rate') {
                if (value < 20) selectedButton = buttons[0];
                else if (value <= 29) selectedButton = buttons[1];
                else selectedButton = buttons[2];
            } else if (groupName === 'oxygen_sat') {
                if (value >= 92) selectedButton = buttons[0];
                else selectedButton = buttons[1];
            } else if (groupName === 'gcs') {
                if (value === 15) selectedButton = buttons[0];
                else selectedButton = buttons[1];
            } else if (groupName === 'urea') {
                // Assuming value is in mmol/L
                if (value < 7) selectedButton = buttons[0];
                else if (value < 14) selectedButton = buttons[1];
                else selectedButton = buttons[2];
            } else if (groupName === 'crp') {
                // Assuming value is in mg/L
                if (value < 50) selectedButton = buttons[0];
                else if (value < 100) selectedButton = buttons[1];
                else selectedButton = buttons[2];
            }
        ...
        try {
            ...
            if (observations[0]) selectButtonByValue('resp_rate', observations[0].value);
            if (observations[1]) selectButtonByValue('oxygen_sat', observations[1].value);
            if (observations[2]) selectButtonByValue('gcs', observations[2].value);
            if (observations[3]) selectButtonByValue('urea', observations[3].value);
            if (observations[4]) selectButtonByValue('crp', observations[4].value);
