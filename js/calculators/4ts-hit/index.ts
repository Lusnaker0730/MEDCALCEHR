...
export const hepScore: Calculator = {
    id: '4ts-hit',
    ...
    initialize: async (client: FHIRClient, patient: Patient, container: HTMLElement): Promise<void> => {
        ...
        const criteria: Record<string, HepCriteria> = {
        ...
        const calculateScore = () => {
            ...
            const resultBox = container.querySelector('#hep-score-result') as HTMLElement;
        ...
        const renderCriteria = (onsetType = 'typical') => {
            ...
            Object.entries(criteria).forEach(([key, data]) => {
                ...
                if (data.options) {
                    ...
                    options = [
                        { label: `No (${data.no})`, value: data.no || '0', checked: true },
                        { label: `Yes (${(data.yes && parseInt(data.yes) > 0) ? '+' : ''}${data.yes})`, value: data.yes || '0' }
                    ];
                }
        ...
        onsetInputs.forEach(radio => {
            radio.addEventListener('change', (e) => {
                renderCriteria((e.target as HTMLInputElement).value);
            });
        });
