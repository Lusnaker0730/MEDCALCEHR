...
export const curb65: Calculator = {
    ...
    initialize: function (client: FHIRClient, patient: Patient, container: HTMLElement): void {
        ...
        const calculate = () => {
            ...
            const resultBox = container.querySelector('#curb65-result') as HTMLElement;
        ...
        if (client) {
            ...
            client.request(`Observation?patient=${client.patient.id}&code=85353-1&_sort=-date&_count=1`)
                .then((response: any) => {
                    if (response.entry && response.entry.length > 0) {
                        ...
                        const rrComp = vitals.component.find((c: any) => c.code.coding[0].code === LOINC_CODES.RESPIRATORY_RATE);
                        const sbpComp = vitals.component.find((c: any) => c.code.coding[0].code === LOINC_CODES.SYSTOLIC_BP);
                        const dbpComp = vitals.component.find((c: any) => c.code.coding[0].code === LOINC_CODES.DIASTOLIC_BP);
            ...
            getMostRecentObservation(client, LOINC_CODES.BUN).then((obs: Observation | null) => {
                if (obs && obs.valueQuantity && obs.valueQuantity.value > 19) {
