# SMART on FHIR 完整指南

本指南整合了 SMART on FHIR 的流程说明和启动步骤。

---

## 📊 目录

1. [快速开始](#快速开始)
2. [认证流程](#认证流程)
3. [启动方式](#启动方式)
4. [关键文件](#关键文件)
5. [常见问题](#常见问题)

---

## 🚀 快速开始

### 最简单的测试方式

1. **启动本地服务器**

    ```bash
    cd MEDCALCEHR
    python -m http.server 8000
    # 或
    npm start
    ```

2. **访问 SMART Launcher**
    - 打开：https://launch.smarthealthit.org/
    - App Launch URL: `http://localhost:8000/launch.html`
    - 选择一个测试患者
    - 点击 "Launch"

3. **完成！**
    - 应用将自动完成认证流程
    - 显示患者信息和计算器列表

---

## 📊 认证流程详解

### 完整认证流程图

```
┌─────────────────────────────────────────────────────────────────┐
│                    SMART on FHIR 認證流程                        │
└─────────────────────────────────────────────────────────────────┘

步驟 1: 從 EHR 或 SMART Launcher 啟動
┌──────────────────┐
│  EHR / Launcher  │
│                  │
│ 點擊啟動應用程式  │
└────────┬─────────┘
         │
         │ 重定向到 launch.html
         │ 帶參數: ?iss=...&launch=...
         ▼
┌──────────────────────────────────────────────────────┐
│  launch.html                                         │
│  http://localhost:8000/launch.html?iss=...&launch=...│
│                                                       │
│  載入 FHIR Client 庫                                  │
│  執行: FHIR.oauth2.authorize({                        │
│    client_id: 'my-app',                              │
│    scope: 'launch/patient patient/*.read',           │
│    redirect_uri: './index.html'                      │
│  })                                                  │
└────────┬─────────────────────────────────────────────┘
         │
         │ 觸發 OAuth2 授權請求
         ▼
┌──────────────────┐
│  EHR 授權伺服器   │
│                  │
│  用戶登入/授權    │
│  (測試環境自動)   │
└────────┬─────────┘
         │
         │ 授權成功，重定向回 index.html
         │ 帶參數: ?code=...&state=...
         ▼
┌──────────────────────────────────────────────────────┐
│  index.html                                          │
│  http://localhost:8000/index.html?code=...&state=... │
│                                                       │
│  檢測到 state 參數 → 跳過重定向邏輯                   │
│  載入 FHIR Client 庫                                  │
│  執行: FHIR.oauth2.ready()                           │
└────────┬─────────────────────────────────────────────┘
         │
         │ 交換授權碼獲取存取令牌
         │ 初始化 FHIR 客戶端
         ▼
┌──────────────────────────────────────────────────────┐
│  FHIR 客戶端就緒                                      │
│                                                       │
│  ✓ 已取得存取令牌                                     │
│  ✓ 已取得病患上下文 (patient.id)                      │
│  ✓ 可以存取 FHIR API                                 │
└────────┬─────────────────────────────────────────────┘
         │
         │ 呼叫 displayPatientInfo(client, div)
         ▼
┌──────────────────────────────────────────────────────┐
│  API 請求: GET /Patient/{id}                          │
│  → 取得病患基本資料 (姓名、生日、性別)                 │
│                                                       │
│  API 請求: GET /Observation?patient={id}&code=...     │
│  → 取得實驗室數據 (需要時)                            │
└────────┬─────────────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────────────────────┐
│  顯示病患資訊與計算器                                  │
│  ┌────────────────────────────────────────────────┐ │
│  │ 👤 病患資訊                                     │ │
│  │ 姓名: John Doe                                 │ │
│  │ 出生日期: 1980-01-01 (年齡: 45)                │ │
│  │ 性別: Male                                     │ │
│  └────────────────────────────────────────────────┘ │
│                                                       │
│  ✓ 可以使用所有計算器                                 │
│  ✓ 計算器可以自動填入病患數據                          │
│  ✓ 可以查詢實驗室數據                                 │
└───────────────────────────────────────────────────────┘
```

---

## 🔌 启动方式

### 方式一：SMART Health IT 沙盒（推荐用于测试）

这是最简单的测试方式，使用公开的沙盒环境。

#### 步骤：

1. **访问 SMART Launcher**
    - 前往：https://launch.smarthealthit.org/

2. **配置启动参数**

    ```
    App Launch URL: http://localhost:8000/launch.html
    Launch Type: Provider EHR Launch
    Patient: 选择任何测试患者
    Provider: 选择任何测试医生
    FHIR Version: R4
    ```

3. **点击 "Launch"**
    - 系统会自动完成 OAuth2 流程
    - 重定向回您的应用

4. **验证成功**
    - 应显示患者信息
    - 所有计算器可用
    - 可自动填入 FHIR 数据

### 方式二：本地 FHIR 服务器

如果您有本地 FHIR 服务器：

1. **启动 FHIR 服务器**

    ```bash
    # 例如使用 HAPI FHIR
    docker run -p 8080:8080 hapiproject/hapi:latest
    ```

2. **修改 launch.html**

    ```javascript
    const defaultIss = 'http://localhost:8080/fhir';
    ```

3. **从 EHR 启动**
    - 使用您的 EHR 系统的 SMART 启动功能
    - 配置 App Launch URL: `http://localhost:8000/launch.html`

### 方式三：Docker 环境

使用 Docker 运行应用：

```bash
cd MEDCALCEHR
docker-compose up -d

# 访问
http://localhost:8080/launch.html
```

**注意**：Docker 环境需要确保应用可从外部访问（SMART Launcher 需要重定向到您的应用）。

---

## 🔑 关键文件说明

### 1. launch.html

**目的**：启动 OAuth2 认证流程

**何时使用**：

- 从 EHR 系统启动应用时
- 首次访问需要患者上下文时

**核心代码**：

```javascript
FHIR.oauth2.authorize({
    client_id: 'my-app',
    scope: 'launch/patient patient/*.read openid fhirUser',
    redirect_uri: './index.html'
});
```

### 2. index.html

**目的**：应用主页面，显示计算器列表

**认证检查逻辑**：

```javascript
// 如果没有 OAuth state 且没有已存在的 session，重定向到 launch
if (!/state=/.test(location.search) && !sessionStorage.getItem('SMART_KEY')) {
    window.location.href = 'launch.html';
}
```

**初始化 FHIR 客户端**：

```javascript
FHIR.oauth2
    .ready()
    .then(client => {
        displayPatientInfo(client, patientInfoDiv);
    })
    .catch(error => {
        console.log('FHIR client not ready, using offline mode');
    });
```

### 3. calculator.html

**目的**：显示单个计算器

**行为**：

- 尝试初始化 FHIR 客户端
- 如果成功，自动填入患者数据
- 如果失败，仍可手动输入使用

### 4. utils.js

**目的**：提供 FHIR 数据访问工具函数

**主要函数**：

```javascript
// 获取观察值
getMostRecentObservation(client, '8480-6'); // 收缩压

// 获取患者条件
getPatientConditions(client, ['38341003']); // 高血压

// 显示患者信息
displayPatientInfo(client, divElement);
```

---

## 🔄 URL 参数说明

### 启动时 (launch.html)

```
http://localhost:8000/launch.html?iss=FHIR_SERVER&launch=LAUNCH_TOKEN
```

- `iss`: FHIR 服务器的基础 URL
- `launch`: EHR 提供的启动令牌（包含患者上下文）

### 认证后 (index.html)

```
http://localhost:8000/index.html?code=AUTH_CODE&state=STATE_TOKEN
```

- `code`: 授权码（用于交换访问令牌）
- `state`: 状态令牌（防止 CSRF 攻击）

---

## 💾 Session Storage

FHIR 客户端使用 sessionStorage 存储：

```javascript
{
    'SMART_KEY': {
        serverUrl: 'FHIR服务器URL',
        tokenResponse: {
            access_token: '访问令牌',
            patient: '患者ID',
            expires_in: 3600
        }
    },
    'patientData': '患者资料的 JSON 字符串'
}
```

**检查方式**：

1. 开启开发者工具 (F12)
2. Application → Session Storage → http://localhost:8000
3. 查看 `SMART_KEY` 和 `patientData`

---

## 🧪 测试场景

### 场景 1: 正常启动（有 SMART 认证）

```
1. SMART Launcher → launch.html?iss=...&launch=...
2. OAuth2 认证
3. index.html?code=...&state=...
4. 客户端初始化成功
5. 显示患者信息 ✓
```

### 场景 2: 直接访问（无认证）

```
1. 直接访问 index.html
2. 检查没有 state 参数
3. 检查没有 SMART_KEY
4. 自动重定向到 launch.html
5. 使用默认 iss 启动认证
```

### 场景 3: 已有 Session

```
1. 访问 index.html
2. 检查到 sessionStorage 有 SMART_KEY
3. 不重定向，直接初始化
4. 使用缓存的患者资料 ✓
```

### 场景 4: 离线模式

```
1. 直接访问 calculator.html?id=bmi-bsa
2. FHIR 客户端初始化失败
3. 显示 "FHIR client not ready"
4. 仍可手动输入使用计算器 ✓
```

---

## ❓ 常见问题

### Q: "FHIR client not ready" 是什么意思？

**原因**：

- 没有经过 OAuth2 认证流程
- Session 已过期
- 认证失败

**解决方法**：

1. 清除 sessionStorage（F12 → Application → Clear）
2. 重新从 SMART Launcher 启动
3. 或直接手动输入数据使用计算器

### Q: 为什么访问 launch.html 显示 404？

**原因**：

- Docker 容器不包含 launch.html
- 文件路径不正确

**解决方法**：

```bash
# 重新构建 Docker 镜像
docker-compose down
docker-compose build --no-cache
docker-compose up -d
```

### Q: 如何测试不同的患者？

在 SMART Launcher 中：

1. 点击 "Change Patient"
2. 选择不同的测试患者
3. 重新启动应用

### Q: 计算器没有自动填入数据？

**检查清单**：

1. ✓ FHIR 客户端已初始化？（查看控制台）
2. ✓ 患者有相关的观察值？（部分测试患者数据不完整）
3. ✓ LOINC 代码正确？（参考 `js/fhir-codes.js`）
4. ✓ 网络请求成功？（查看 Network 标签）

### Q: 如何在生产环境使用？

1. **注册应用**：在您的 EHR 系统注册 SMART 应用
2. **获取 client_id**：从 EHR 获取
3. **配置域名**：使用 HTTPS 和正式域名
4. **更新 redirect_uri**：指向生产环境 URL
5. **配置 launch.html**：使用生产环境的 client_id

---

## 📚 延伸阅读

- [SMART App Launch 规范](http://hl7.org/fhir/smart-app-launch/)
- [FHIR Client JavaScript 文档](https://github.com/smart-on-fhir/client-js)
- [OAuth 2.0 授权码流程](https://oauth.net/2/grant-types/authorization-code/)
- [LOINC 代码查询](https://loinc.org/)
- [SNOMED CT 浏览器](https://browser.ihtsdotools.org/)

---

## 🔧 开发提示

### 查看 FHIR 请求

打开浏览器开发者工具：

```
F12 → Network → Fetch/XHR
```

所有 FHIR API 请求都会显示在这里。

### 调试认证流程

在 `launch.html` 和 `index.html` 中添加：

```javascript
console.log('Current URL:', window.location.href);
console.log('Session Storage:', sessionStorage.getItem('SMART_KEY'));
```

### 健康检查

访问：`http://localhost:8000/health-check.html`

自动检查：

- ✓ FHIR 客户端状态
- ✓ Session Storage
- ✓ 患者数据
- ✓ 计算器加载

---

**提示**：遇到问题？查看浏览器控制台（F12）获取详细错误信息！
