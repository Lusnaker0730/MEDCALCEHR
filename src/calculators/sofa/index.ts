...
export const sofaConfig: ScoringCalculatorConfig = {
    ...
    customInitialize: async (client, patient, container, calculate): Promise<void> => {
        ...
        try {
            // Platelets (Lowest is worst)
            const plateletsResult = await fhirDataService.getAggregatedObservation(
        ...
        try {
            // Creatinine (Highest is worst)
            const creatinineResult = await fhirDataService.getAggregatedObservation(
        ...
        try {
            // Bilirubin (Highest is worst)
            const bilirubinResult = await fhirDataService.getAggregatedObservation(
