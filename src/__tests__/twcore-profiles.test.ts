...
describe('TW Core Profiles', () => {
    describe('TW_CORE_PROFILES URLs', () => {
        test('should have correct base URL for all profiles', () => {
            for (const [key, url] of Object.entries(TW_CORE_PROFILES)) {
                expect(url).toMatch(
                    /^https:\/\/twcore\.mohw\.gov\.tw\/ig\/twcore\/StructureDefinition\//
                );
            }
        });
        ...
        test('should have MedicationRequest profile URL', () => {
            expect(TW_CORE_PROFILES.MedicationRequest).toBe(
                `${TW_CORE_BASE}/MedicationRequest-twcore`
            );
        });
    ...
    describe('getTWCoreObservationProfile()', () => {
        ...
        test.each([
            ...
            expect(getTWCoreObservationProfile(loincCode)).toBe(
                TW_OBSERVATION_PROFILES.laboratoryResult
            );
        });
        ...
        test('should handle comma-separated codes (take first)', () => {
            expect(getTWCoreObservationProfile('85354-9,55284-4')).toBe(
                TW_OBSERVATION_PROFILES.bloodPressure
            );
        });
    });
