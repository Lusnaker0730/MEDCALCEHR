...
const mockGetTimeoutMinutes = jest.fn<any>().mockReturnValue(15);
const mockSetEffectiveTimeout = jest.fn<any>();
const mockOnLogout = jest.fn<any>();
const mockLogout = jest.fn<any>().mockResolvedValue(undefined);
...
describe('TokenLifecycleManager', () => {
    ...
    beforeEach(async () => {
        ...
        delete (window as any).MEDCALC_CONFIG;
    ...
    describe('Token state extraction', () => {
        test('should extract expiresAt when present in client state', () => {
            const client = createMockClient({ expiresAt: Date.now() + 300_000 });
    ...
    describe('Session timeout sync', () => {
        test('should set effective timeout when token TTL < session timeout', () => {
            ...
            const client = createMockClient({ expiresAt: Date.now() + 5 * 60_000 });
            tokenLifecycleManager.initialize(client);
        ...
        test('should not change timeout when token TTL > session timeout', () => {
            ...
            const client = createMockClient({ expiresAt: Date.now() + 20 * 60_000 });
            tokenLifecycleManager.initialize(client);
    ...
    describe('Token clock', () => {
        ...
        test('should not show warning when token has plenty of time', () => {
            const client = createMockClient({ expiresAt: Date.now() + 600_000 });
            tokenLifecycleManager.initialize(client);
    ...
    describe('Token refresh', () => {
        test('should update state on successful refresh', async () => {
            ...
            const client = createMockClient({ expiresAt: Date.now() + 100_000 });
    ...
    describe('Visibility change', () => {
        test('should check token expiry when tab becomes visible', () => {
            const client = createMockClient({ expiresAt: Date.now() + 60_000 });
            tokenLifecycleManager.initialize(client);
    ...
    describe('Disable via config', () => {
        test('should not start token clock when disableTokenLifecycle is true', async () => {
            ...
            const client = createMockClient({ expiresAt: Date.now() + 30_000 });
            manager.initialize(client);
