...
function readCalcSource(calcId: string): string | null {
...
function detectFactoryType(source: string): 'formula' | 'scoring' | 'dynamic-list' | 'unknown' {
    if (source.includes('createScoringCalculator')) return 'scoring';
    if (source.includes('createDynamicListCalculator')) return 'dynamic-list';
    if (source.includes('createUnifiedFormulaCalculator')) return 'formula';
    return 'unknown';
}

/** Extract number input field IDs and check for validation rules */
function checkNumberInputValidation(source: string): {
    fieldId: string;
    hasValidation: boolean;
}[] {
    const results: { fieldId: string; hasValidation: boolean }[] = [];
    ...
    for (const fieldId of allIds) {
        ...
        if (!hasUnitValidation) {
            ...
            const widerPattern = new RegExp(
                `id:\\s*'${escapedId}'[\\s\\S]{0,500}(?:unitConfig|unitToggle):\\s*\\{[^}]*type:\\s*['"][^'"]+['"]`,
                's'
            );
