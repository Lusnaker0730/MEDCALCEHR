...
const mockGetObservationValue = jest.fn<any>();
const mockGetPatientConditions = jest.fn<any>();
const mockGetMedicationRequests = jest.fn<any>();
const mockCalculateAge = jest.fn<any>();
const mockIsRestrictedResource = jest.fn<any>();

const mockGetLoincName = jest.fn<any>();
const mockGetMeasurementType = jest.fn<any>();
const mockIsValidLoincCode = jest.fn<any>();

const mockGetTextNameByLoinc = jest.fn<any>();

const mockGetCachedObservation = jest.fn<any>();
const mockCacheObservation = jest.fn<any>();
const mockClearPatientCache = jest.fn<any>();

const mockCheckStaleness = jest.fn<any>();
...
const mockUnitConvert = jest.fn<any>();
...
const mockRecordCalculation = jest.fn<any>();
const mockRecordDerivation = jest.fn<any>();
const mockGetProvenanceForTarget = jest.fn<any>();
const mockGenerateLineageReport = jest.fn<any>();
...
const MOCK_LOINC_CODES: Record<string, string> = {
    SYSTOLIC_BP: '8480-6',
    DIASTOLIC_BP: '8462-4',
...
function createMockClient(requestFn?: jest.Mock<any>) {
    const req = requestFn || jest.fn<any>();
    return {
        patient: {
            id: 'test-patient',
            read: jest.fn<any>(),
...
function makeBPPanelObservation(systolic: number, diastolic: number, dateStr?: string) {
    return {
...
function makeObservation(value: number, unit: string, code: string, dateStr?: string) {
    return {
        resourceType: 'Observation',
        id: `obs-${code}`,
...
describe('FHIRDataService -- extended coverage', () => {
    let FHIRDataService: any;
    let createFHIRDataService: any;
    ...
    describe('getObservation -- caching', () => {
        ...
        test('logs audit event after successful fetch', async () => {
            ...
            await svc.getObservation('29463-7');
            expect(mockLogResourceRead).toHaveBeenCalledWith(
                'Observation',
                'obs-weight-1',
                'code=29463-7'
            );
    ...
    describe('getObservation -- text query', () => {
        ...
        test('handles empty entry in text query response', async () => {
            mockIsValidLoincCode.mockReturnValue(true);
            const mockReq = jest.fn<any>().mockResolvedValue({ entry: [] });
            const client = createMockClient(mockReq as jest.Mock<any>);
    ...
    describe('getBloodPressure', () => {
        ...
        test('extracts date from issued field as fallback', async () => {
            const bpObs: any = makeBPPanelObservation(120, 80);
    ...
    describe('autoPopulateInput', () => {
        ...
        test('formats to specified decimal places', async () => {
            ...
            const inputEl = container.querySelector('#creatinine') as HTMLInputElement;
    ...
    describe('autoPopulateFields', () => {
        ...
        test('handles BP special case: populates systolic and diastolic from panel', async () => {
            ...
            const systolicInput = container.querySelector('#systolic') as HTMLInputElement;
            const diastolicInput = container.querySelector('#diastolic') as HTMLInputElement;
        ...
        test('generates correct loaded/missing summary', async () => {
            ...
            mockGetMostRecentObservation.mockImplementation(async (_client: any, code: string) => {
                if (code === '29463-7') return weightObs;
                return null;
            });
            mockGetObservationValue.mockImplementation((obs: any) => {
    ...
    describe('getObservations', () => {
        test('fetches multiple observations and returns a Map', async () => {
            mockGetMostRecentObservation.mockImplementation(async (_client: any, code: string) => {
                if (code === '29463-7') return makeObservation(70, 'kg', '29463-7');
                if (code === '8302-2') return makeObservation(170, 'cm', '8302-2');
                return null;
            });
            mockGetObservationValue.mockImplementation((obs: any) => {
    ...
    describe('getAllObservations', () => {
        ...
        test('returns empty array on error', async () => {
            const mockReq = jest.fn<any>().mockRejectedValue(new Error('Server error'));
            const client = createMockClient(mockReq as jest.Mock<any>);
    ...
    describe('getObservationsInWindow', () => {
        ...
        test('returns empty array on error', async () => {
            const mockReq = jest.fn<any>().mockRejectedValue(new Error('err'));
            const client = createMockClient(mockReq as jest.Mock<any>);
    ...
    describe('recordCalculationProvenance', () => {
        test('records calculation provenance', async () => {
            ...
            await svc.recordCalculationProvenance(
                'calc-1',
                'BMI Calculator',
        ...
        test('records derivation when source observations provided', async () => {
            ...
            await svc.recordCalculationProvenance(
                ...
                { weight: 70 },
                { bmi: 24.2 },
                ['Observation/obs-weight-1']
            );

            expect(mockRecordDerivation).toHaveBeenCalledWith(
                expect.stringContaining('#calculation-calc-1-'),
                'BMI Calculator Result',
                [{ reference: 'Observation/obs-weight-1', display: 'Source observation' }],
                'Calculated using BMI Calculator'
            );
        });
