...
jest.mock('../audit-event-service', () => {
    const logCalculation = jest.fn<() => Promise<void>>().mockResolvedValue(undefined);
    return {
...
describe('Unified Formula Calculator — Audit Integration', () => {
    ...
    function setupSimpleCalculator(options?: {
        calculate?: (v: Record<string, any>) => { label: string; value: string }[];
    }) {
    ...
    it('should call logCalculation after debounce for a successful simple calculation', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledWith(
            'test-calc',
            'Test Calculator',
            expect.objectContaining({ val: 42 }),
            expect.objectContaining({ Result: '84' }),
            true
        );
    });
    ...
    it('should debounce multiple rapid calculations into a single audit event', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledWith(
            'test-calc',
            'Test Calculator',
            expect.objectContaining({ val: 123 }),
            expect.objectContaining({ Result: '246' }),
            true
        );
    });
    ...
    it('should call logCalculation immediately (no debounce) on calculation failure', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledWith(
            ...
            'Test Calculator',
            expect.objectContaining({ val: 42 }),
            expect.objectContaining({ error: 'Error: Calculation failed' }),
            false
        );
    });
    ...
    it('should not break calculation when audit logging fails', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledTimes(1);
        const resultBox = container.querySelector('.ui-result-content') as HTMLElement;
    ...
    it('should call logCalculation after debounce for a successful complex calculation', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ score: 75, interpretation: 'High' }),
            true
        );
    });
    ...
    it('should call logCalculation immediately on complex calculation failure', () => {
        ...
        expect(mockLogCalculation).toHaveBeenCalledWith(
            ...
            'Test Complex Calculator',
            {},
            expect.objectContaining({ error: 'Error: Complex calc error' }),
            false
        );
    });
});
