...
function createMockCache(): Cache {
    ...
    return {
        ...
        match: jest.fn(async (request: RequestInfo | URL) => {
            const url = typeof request === 'string' ? request : (request as Request).url;
            const resp = store.get(url);
            return resp ? resp.clone() : undefined;
        }),
        delete: jest.fn(async (request: RequestInfo | URL) => {
            const url = typeof request === 'string' ? request : (request as Request).url;
...
function createMockCacheStorage(): CacheStorage {
    ...
    return {
        ...
        delete: jest.fn(async (cacheName: string) => {
            return caches.delete(cacheName);
        }),
        keys: jest.fn(async () => {
            return Array.from(caches.keys());
        }),
        has: jest.fn(async (cacheName: string) => {
...
describe('Cache Manager Module', () => {
    ...
    describe('CacheManager — with mocked Cache API', () => {
        ...
        test('cleanExpired should keep non-expired entries', async () => {
            ...
            (cache.delete as jest.Mock).mockClear();
        ...
        test('getCacheSize should return 0 on Cache API error', async () => {
            const consoleSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
            (mockCacheStorage.open as jest.Mock<any>).mockRejectedValueOnce(new Error('fail'));
        ...
        test('clearAllCaches should handle Cache API errors gracefully', async () => {
            const consoleSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
            (mockCacheStorage.keys as jest.Mock<any>).mockRejectedValueOnce(new Error('fail'));
    ...
    describe('FHIRCacheManager', () => {
        let fhirManager: FHIRCacheManager;
    ...
    describe('FHIRCacheManager — clearPatientCache (with Cache API)', () => {
        ...
        test('clearPatientCache should remove patient and observation entries for given patient', async () => {
            ...
            await cache.put(
                `http://localhost/patient-P100`,
                new Response(JSON.stringify({ data: 'patient data' }))
            );
            await cache.put(
                `http://localhost/observation-P100-2160-0`,
                new Response(JSON.stringify({ data: 'obs data' }))
            );
            await cache.put(
                `http://localhost/patient-P200`,
                new Response(JSON.stringify({ data: 'other patient' }))
            );

            await fhirManager.clearPatientCache('P100');

            // cache.delete should have been called for P100 entries
            const deleteCalls = (cache.delete as jest.Mock).mock.calls;
            const deletedUrls = deleteCalls.map((call: any) => {
                const arg = call[0];
                return typeof arg === 'string' ? arg : arg.url;
            });

            expect(deletedUrls.some((url: string) => url.includes('patient-P100'))).toBe(true);
            expect(deletedUrls.some((url: string) => url.includes('observation-P100'))).toBe(true);
        });
    ...
    describe('CalculatorCacheManager', () => {
        let calcManager: CalculatorCacheManager;
    ...
    describe('StaticCacheManager', () => {
        let staticManager: StaticCacheManager;
    ...
    describe('StaticCacheManager — cacheStaticResources (with Cache API)', () => {
        ...
        test('cacheStaticResources should not throw when Cache API is unavailable', async () => {
            ...
            await expect(
                noApiManager.cacheStaticResources(['/css/main.css'])
        ...
        test('cacheStaticResources should handle errors gracefully', async () => {
            ...
            await expect(
                staticManager.cacheStaticResources(['/test.css'])
            ).resolves.toBeUndefined();
