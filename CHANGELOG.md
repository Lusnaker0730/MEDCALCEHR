...
- **Formula/list/table HTML rendering**: `createFormulaSection`, `createList`, `createTable` now use `sanitizeHTML()` instead of `escapeHTML()` to preserve intentional `<sub>`, `<sup>`, `<strong>`, `<span>`, `<br>` formatting used by 10+ calculators (`ui-builder.ts`)
- **Audit `clearLocalEvents` bypassed encrypted storage**: Was calling `localStorage.removeItem()` directly, never clearing AES-GCM encrypted data; now uses `secureLocalStore()` to properly clear (`audit-event-service.ts`)
- **Audit `loadPendingEvents` race condition**: Constructor fire-and-forget async load was overwriting `eventQueue`, dropping events recorded before load completed; now merges (`audit-event-service.ts`)
- **Audit `sanitizeForAudit` crash on non-object input**: Added defensive guard for primitive values (`audit-event-service.ts`)
- **Provenance `clearLocalRecords` bypassed encrypted storage**: Same fix as audit service — uses `secureLocalStore()` instead of raw `localStorage.removeItem()` (`provenance-service.ts`)
- **Provenance `loadPendingRecords` race condition**: Same merge fix as audit service (`provenance-service.ts`)
- **Provenance `sanitizeData` incomplete PHI coverage**: Expanded sensitive fields from 6 to 22 (was missing `name`, `birthdate`, `address`, `phone`, `email`, `mrn`, `nationalid`, `photo`); added non-object guard and array recursion (`provenance-service.ts`)
- **`escapeHTML` per-call allocations**: Hoisted `htmlEscapeMap`, `HTML_ESCAPE_RE`, and `NULL_BYTE_RE` to module scope in `security.ts` to avoid ~150 unnecessary object allocations per component render
